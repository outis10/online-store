...
export const OrderItem = (props: IOrderItemProps) => {
  ...
  const {orderItemList, match, loading, totalItems, isAdmin} = props;
  return (
    <div>
          ...
          <Table responsive>
            ...
            <tr>
              ...
              <th className="hand" onClick={sort('totalPrice')}>
                <Translate contentKey="storeApp.orderItem.totalPrice">Total Price</Translate> <FontAwesomeIcon
                icon="sort"/>
              </th>
            ...
            {orderItemList.map((orderItem, i) => (
              <tr key={`entity-${i}`}>
                ...
                <td>{orderItem.product ?
                  <Link to={`product/${orderItem.product.id}`}>{orderItem.product.name}</Link> : ''}</td>
                <td>{orderItem.order ?
                  <Link to={`product-order/${orderItem.order.id}`}>{orderItem.order.code}</Link> : ''}</td>
